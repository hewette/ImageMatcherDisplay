namespace ImageMatcherDisplay
{
    ...
    public class GridViewHelper
    {
        ...
        public void PrepareGrid(Grid ImageGrid, List<ImageFile> ListImageFile)
        {
            ...
            for (int i = 0; i < this._imageMatcherConfig.NumberofColumnsInImageGrid; i++)
            {
                ImageGrid.ColumnDefinitions.Add(new ColumnDefinition());
            }

            int imageRows = (int)Math.Ceiling((float)ListImageFile.Count / (float)this._imageMatcherConfig.NumberofColumnsInImageGrid);
            ...
            for (int iRow = 0; iRow < imageRows; iRow++)
            {
                ...
                for (int iColumn = 0; iColumn < this._imageMatcherConfig.NumberofColumnsInImageGrid; iColumn++)
                {
                    if (currentImageNo > ListImageFile.Count - 1)
                    {
                        break;
                    }
                    ...
                    btn.Click += new RoutedEventHandler(this.GridClickEventHandler);
